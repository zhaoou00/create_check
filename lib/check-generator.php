	...
	function PrintChecks() {
		...
		foreach( $this->checks as $check ) {
			...
			if( array_key_exists('logo',$check) && $check['logo'] != "" ) {
				...
				$pdf->Image($check['logo'], $x + $cell_left, $y + $cell_top + .12*$ry, $logo_width);
			}

			$pdf->SetFont('Twcen','',7);

			// name
			$pdf->SetXY( $x + $cell_left + $logo_offset, $y + $cell_top + .1*$ry );
			...
			// date
			$pdf->Line( $x + 3.5*$rx, $y + .58*$ry, $x + 3.5 *$rx+ 1.2*$rx, $y + .58 *$ry);
			$pdf->SetXY( $x + 3.5*$rx, $y + .48*$ry);
			...
			$pdf->SetXY( $x + $cell_left, $y + 2.125*$ry);
			...
			// date
			if( $check['date'] != "" ) {
				$pdf->SetXY( $x + 3.5 *$rx+ .3*$rx, $y + .38*$ry);
				$pdf->Cell( 1, .25, $check['date'] );
			}

			// pay to
			if( $check['pay_to'] != "" ) {
				$pdf->SetXY( $x + $cell_left + .5*$rx, $y + .88*$ry);
			...
			if( $check['amount'] > 0 ) {
				...
				$pdf->SetXY( $x + 4.5*$rx + .06*$rx, $y + .83*$ry);
				$pdf->Cell( 1, .25, $amt );
			}
			...
			$pdf->SetXY( $x + $cell_left, $y + 2.47 *$ry);
			$pdf->Cell( 5, (10/72), $routingstring );


			// signature
			if( substr($check['signature'],-3) == 'png' ) {
				$sig_offset = 1.75*$rx;  // width of signature
				$pdf->Image($check['signature'], $x + $cell_left + 3.4*$rx, $y + 1.88*$ry, $sig_offset);
			} else {
				$pdf->SetFont('Arial','i',10);
				if( $check['signature'] != "" ) {
					$pdf->SetXY( $x + $cell_left + 3.4*$rx, $y + 2.01*$ry);
			...
			if( isset($check['pre_auth']) ) {
				$pdf->SetXY( $x + $cell_left + 3.3*$rx, $y + 2.155*$ry);
